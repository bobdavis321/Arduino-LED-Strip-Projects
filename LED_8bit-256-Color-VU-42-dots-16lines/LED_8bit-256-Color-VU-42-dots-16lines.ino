...
// Displays dots on 16 lines of LED's.
...
void readMSGEQ7() { //reset the chip
  ...
  for(int band=0; band < 7; band++) {  //loop thru all 7 bands
    ...
    left0[band] = analogRead(PIN_0)-32;   // store band reading
    right0[band] = analogRead(PIN_1)-32;  // store band reading
    left1[band] = analogRead(PIN_2)-32;   // store band reading
    right1[band] = analogRead(PIN_3)-32;  // store band reading
    left2[band] = analogRead(PIN_4)-32;   // store band reading
    right2[band] = analogRead(PIN_5)-32;  // store band reading
...
void loop() {
    ...
    int space=47;  // number of blanks to roll over to next line
    col=0x00;
    for (int s=0; s<space; s++) {sendPixelRow(col);}
